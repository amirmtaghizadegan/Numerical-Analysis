...
for n = 1:length(t)-1
    ...
    %% for X axis we calculate U = u(n+1/2)
    for i = 1:length(x)
        for j = 1:length(y)
            if i == 1 %BC1
                A(id(i, j), id(i, j)) = 1;
            ...
            elseif j == length(y) %BC4
                ...
                A(id(i, j), id(i, j)) = 1 + lamdaX;
                B(id(i, j)) = 0.5 * lamdaY * (u(i, j+1, n) + ...
                    u(i, j-1, n)) + (1 - lamdaY) * u(i, j, n);
            end
        end
    end
    U = A\B;
    ...
    A = zeros(length(x) * length(y));
    B = zeros(length(x) * length(y), 1);
    %% for Y axis we calculate U = u(n+1)
    for i = 1:length(x)
        for j = 1:length(y)
            if i == 1 %BC1
                A(id(i, j), id(i, j)) = 1;
            ...
            elseif j == length(y) %BC4
                ...
                A(id(i, j), id(i, j)) = 1 + lamdaY;
                B(id(i, j)) = 0.5 * lamdaX * (U(i+1, j) + ...
                    U(i-1, j)) + (1 - lamdaX) * U(i, j);
            end
        end
    end
    U = A\B;
